...
function init(turn) {
  // Remove .activeCells class from all black cells
  ...
  // Apply move function to pieces based on turn
...
function handleMove(target) {
  ...
  if (target.classList.contains('active')) {
    // Remove .active class to clicked piece then revert all .activeCells
    ...
    for (const move of possibleMoves[position][turn]) {
      ...
      // TODO: Add check for valid piece-eating

    }
    console.log(`${position} Selected \nPossible moves: ${possibleMoves[position][turn]}`);
  }
}

function movePieceToCell(target) {
  console.log(`Moved to ${target.dataset.cell}`);
